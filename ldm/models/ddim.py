...
class DDIMWrapper(object):
    ...
    def register_schedule(self, ddim_num_steps):
        ...
        ddim_alphas = alphas_cumprod.cpu()[self.ddim_steps]
    ...
    def p_sample(self, x, ctx, t, idx, cfg_scale = None):
        ...
        a_t = torch.full((bs,1,1,1), alphas[idx], device=device)
        a_prev = torch.full((bs,1,1,1), alphas_prev[idx], device=device)
        sqrt_sub_at = torch.full((bs,1,1,1), sqrt_sub_alphas[idx], device=device)
    ...
    def sample(self, ddim_num_steps, ctx, batch_sz, shape, cfg_scale = None):
        ...
        bs = batch_sz if cfg_scale else batch_sz * 2
