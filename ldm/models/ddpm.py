...
class LatentDiffusion(pl.LightningModule):
    ...
    def get_input_key(self, batch, key):
        ...
        x = x.to(memory_format=torch.contiguous_format).to(self.device, type=torch.float16)
    ...
    def get_input(self, batch, return_first_stage_outputs=True, return_original_cond=True, bs=None):
        ...
        sat_emb = batch["satellite_emb"][:bs].to(self.device, type=torch.float16)
        lat_emb = batch["lat_emb"][:bs].to(self.device, type=torch.float16)
        lng_emb = batch["lng_emb"][:bs].to(self.device, type=torch.float16)
