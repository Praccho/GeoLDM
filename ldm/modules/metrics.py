...
class Metrics(nn.Module):
    def __init__(self, gen_imgs, street_imgs):
        ...
        self.pnr = PrecisionRecall(self.fid.model(gen_imgs), self.fid.model(street_imgs))
        self.precision = self.pnr['precision']
        self.recall = self.pnr['recall']

    def get_metrics(self):
        return {'ssim': self.ssim, 'fid': self.fid.compute(), 
...
class PrecisionRecall(nn.Module):
    def __init__(self, ref_features, eval_features, nhood_sizes=[3],
        ...
        """
        self.state = dict()
        self.num_images = ref_features.shape[0]
        self.num_features = ref_features.shape[1]

        # Initialize DistanceBlock and ManifoldEstimators.
        self.distance_block = DistanceBlock(self.num_features, num_gpus)
        self.ref_manifold = ManifoldEstimator(self.distance_block, ref_features, row_batch_size, col_batch_size, nhood_sizes) 
        self.eval_manifold = ManifoldEstimator(self.distance_block, eval_features, row_batch_size, col_batch_size, nhood_sizes)

    def get_pnr(self):
        # Evaluate precision and recall using k-nearest neighbors.
        print('Evaluating k-NN precision and recall with %i samples...' % self.num_images)
        start = time()

        # Precision: How many points from eval_features are in ref_features manifold.
        precision = self.ref_manifold.evaluate(self.eval_features)
        self.state['precision'] = precision.mean(axis=0)

        # Recall: How many points from ref_features are in eval_features manifold.
        recall = self.eval_manifold.evaluate(self.ref_features)
        self.state['recall'] = recall.mean(axis=0)
...
if __name__ == '__main__':
    ...
    metrics = Metrics(gen_img, street_img)
